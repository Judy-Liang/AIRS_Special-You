...
# from deep_sort_pytorch.utils.io import write_results
...
def vis_res_fast(res, img, class_names, colors, thresh, min_confidence, nms_max_overlap, n):
    ...
    # ---------------------------- change -------------------------------
    # bbox = [x1,y1,x2,y2]
    bbox_xywh = [[bbox[0], bbox[1], bbox[2]-bbox[0], bbox[3]-bbox[1]] for bbox in bboxes]
    ...
    if len(bbox_xywh) > 0:
        if n == 1 or (track_id == None):
            ...
            trackers = tracker.update(bbox_xywh, scores, img)
    ...
    if ins.has("pred_bit_masks"):
        ...
        img = vis_bitmasks_with_classes(
            ...
            draw_contours=False, alpha=0.8, yolov_id = yolov_id, output_mask = True, feather = False
        ) #revise the masks
...
if __name__ == "__main__":
    ...
    # --------------- initialize tracker ---------------
    cfg2 = get_config()
    cfg2.merge_from_file(args.config_deepsort)   # deep sort
    cfg2.USE_FASTREID = False
    global tracker, track_id
    track_id = None
    tracker = build_tracker(cfg2, use_cuda=(cfg.MODEL.DEVICE == "cuda"))    #create instance of the deep-SORT tracker
    # --------------------------------------------------
    ...
    if args.input:
        ...
        while(video.isOpened()):
            ...
            res = vis_res_fast(res, frame, class_names, colors, conf_thresh, args.min_confidence, args.nms_max_overlap, n)
            # cv2.imshow('frame', res)
            #cv2.imshow('frame', res)
            
            cv2.imwrite(os.path.join('video_results/demo_5_org', str(n)+'frame.jpg') ,res) #for jpg
            n += 1
