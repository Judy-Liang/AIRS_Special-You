...
import cv2
import numpy as np
from alfred.vis.image.common import get_unique_color_by_id, get_unique_color_by_id2, get_unique_color_by_id_with_dataset
from alfred.vis.image.det import draw_one_bbox
from PIL import Image
from alfred.vis.image.get_dataset_color_map import *
...
def vis_bitmasks_with_classes(img, classes, bitmasks, 
                            force_colors=None, scores=None, 
                            class_names=None, mask_border_color=None, 
                            draw_contours=False, alpha=0.4, fill_mask=True, 
                            return_combined=True, thickness=2, yolov_id=None, output_mask = False, feather = False, invisible=False, fg=None):
    """
    visualize bitmasks on image
    """
    # need check if img and bitmask with same W,H
    ...
    for i, m in enumerate(bitmasks):
        ...
        if len(cts) > 0:
            if return_combined:
                if fill_mask:
                    ...
                    contours = cv2.drawContours(res_m, [cts], -1,  color=(255,255,255), thickness=-1)
                    kernel = np.ones((15,15),np.uint8)
                    dilation = cv2.dilate(contours, kernel)
                    if output_mask:
                            #out = contours
                            out = img
                    else:
                        ## 只贴一个人：
                        out = np.where(dilation!=np.array([255,255,255]), fg, img)
                    ##############只贴一个人##################
                    '''# Method 3
                    blurred_img = cv2.blur(img, (15, 15), 0) # blur
                    out = img.copy()
                    mask = bitmasks.astype(np.bool)
                    out[mask>0] = blurred_img[mask>0]'''

                    #Method 1
                    #blurred_img = cv2.blur(img, (15, 15), 0)
                    #mask_inverse = np.ones(res_m.shape).astype(np.uint8)*255 - res_m
                    '''print('blurred_img',blurred_img.shape) #(1080, 1920, 3)
                    print('bitmasks',bitmasks) 
                    print('bitmasks_shape',bitmasks.shape) #(11, 338, 600)
                    print('img',img.shape) #(1080, 1920, 3)
                    print('mask_inv', mask_inverse.shape) #(11, 338, 600)'''
                    #img = cv2.bitwise_and(blurred_img, res_m) + cv2.bitwise_and(img, mask_inverse)
                    
                    # Method 2
                    # img = np.where(bitmasks==np.array([255, 255, 255]), img, blurred_img)
                    '''out = img.copy()
                    out = cv2.blur(img, (5, 5), 0)
                    img = out'''
                    ...
                    if not invisible:
                        if not feather:
                            ...
                            blurred_dilation = cv2.GaussianBlur(img, (25, 25), 0)
                            ...
                            if output_mask:
                                #out = contours
                                out = img
                            #cv2.drawContours(img, [cts], -1,  color=(255,255,0), thickness=2)
                            #out = np.where(contours_feather!=np.array([255,255,255]), out, blurred_feather)
                            else:
                                out = np.where(dilation!=np.array([255,255,255]), img, blurred_dilation)
                                out = np.where(contours!=np.array([255,255,255]), out, blurred_img)


                    ########### feather
                    if not invisible:
                        if feather:
                            blurred_img = cv2.blur(img, (35, 35), 0)
                            dst = img
                            obj= blurred_img
                            ...
                            out = cv2.seamlessClone(obj, dst, mask, center, cv2.MIXED_CLONE)
                            #contours = cv2.drawContours(res_m, [cts], -1,  color=(255,255,255), thickness=-1)
                            #out = np.where(contours!=np.array([255,255,255]), mixed_clone, obj)
                    ...
                    if not invisible:
                        ...
                        if output_mask:
                            ...
                            # out = np.where(dilation!=np.array([255,255,255]), img, fg)
                            ## 只贴一个人-要改的：
                            out = np.where(dilation!=np.array([255,255,255]), fg, img)

                    ########## 替换一个人 ###########
                        '''original_res = cv2.bitwise_and(fg,fg,mask=
                    ...
                    if draw_contours:
                        if mask_border_color:
                            c = mask_border_color
                        cv2.drawContours(out, [cts], -1,  color=(255,255,0),
                                        thickness=thickness, lineType=cv2.LINE_AA)
                else:
                    cv2.drawContours(out, [cts], -1,  color=c,
                                    thickness=thickness, lineType=cv2.LINE_AA)
            else:
                print('else!!!')
                if fill_mask:
        ...
        if classes is not None:
            ...
            if len(cts) > 0:
                ...
                cy = int(M["m01"] / M["m00"])
                # draw labels 
                #cv2.putText(out, txt, (cx, cy), font, font_scale, [255, 255, 255], 1, cv2.LINE_AA) # img
    if return_combined:
        #img = cv2.addWeighted(img, 0.7, res_m, alpha, 0.4) #img
        #img = cv2.addWeighted(img, 0.7, out, alpha, 0.4)
        '''blurred_img = cv2.blur(img, (15, 15), 0)
        mask_inverse = np.ones(bitmasks.shape).astype(np.uint8)*255 - bitmasks
        print('blurred_img',blurred_img.shape) #(1080, 1920, 3)
        print('bitmasks',bitmasks) 
        print('bitmasks_shape',bitmasks.shape) #(11, 338, 600)
        print('img',img.shape) #(1080, 1920, 3)
        ...
        # Method 1
        # Different shape
        final_image = cv2.bitwise_and(blurred_img, bitmasks) + cv2.bitwise_and(img, mask_inverse)
